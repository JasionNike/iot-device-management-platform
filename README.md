...
| 地址 | 说明 |
|------|------|
| http://localhost:8080 | 前端首页（账号见下方） |
...
| 用户名 | 密码 | 角色 | 说明 |
|--------|------|------|------|
| `admin` | `admin123` | 管理员 | 全部权限：设备管理、产品管理、OTA升级、告警处理 |
| `operator` | `oper123` | 运维工程师 | 设备管理、告警处理、OTA升级 |
| `viewer` | `view123` | 只读用户 | 仅查看权限，不可操作 |

### 数据库连接

> ⚠️ **安全策略**：MySQL 仅监听 ECS 本地 `127.0.0.1:3306`，不暴露公网。**只有通过 SSH 隧道才能连接**，外人无法直接访问。
...
| 常规 标签页 | |
|------|------|
| 主机 | `127.0.0.1` ← **注意：不是47.108.86.221！** |
| 端口 | `3306` |
| 用户名 | `root` |
| 密码 | `IoTPlatform2026` |

**DBeaver / DataGrip** 同理，在 SSH/SSL 标签中配置隧道，常规页主机填 `127.0.0.1`。

**命令行隧道：**

```bash
ssh -L 3307:127.0.0.1:3306 root@47.108.86.221 -N
# 建立后连接 127.0.0.1:3307 即可
```
