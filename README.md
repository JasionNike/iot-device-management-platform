...
> ⚠️ **安全策略**：MySQL 仅监听 ECS 本地 `127.0.0.1:3306`，不暴露公网。需通过 SSH 隧道连接。
...
```bash
ssh -L 3306:127.0.0.1:3306 root@47.108.86.221 -N
```

建立隧道后，本地 Navicat / DBeaver / DataGrip 连接 `localhost:3306` 即可。
